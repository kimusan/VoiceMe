...
- On-device ASR using sherpa-onnx v1.13.3 with 16 KB-aligned arm64 native libraries, the Parakeet TDT v3 int8 multilingual pack as the default candidate, plus compact fallback models, experimental custom Whisper tiny/base sherpa-onnx candidates with preferred-language hints, and runtime-readiness gates before any archive is treated as dictation-ready.
- Optional later engines/model packs: a dedicated whisper.cpp Android runtime if it materially beats sherpa Whisper on-device, fp32 Parakeet benchmark packs on capable devices, and/or Vosk fallback.
...
The project has an Android/Kotlin/Compose prototype with interactive Material 3 setup, settings, compact model/language profiles with Custom model management, History, and About screens. Onboarding/settings/model choices are persisted locally. The app can download and verify sherpa-onnx model archives with a Material progress bar and percentage text, unpack prepared runtime files, start a foreground local `AudioRecord` + sherpa-onnx dictation session from the floating overlay, run experimental local live insertion for both streaming models and chunked offline Danish-capable models, and local SmolLM2 GGUF-based Fix correction with deterministic fallback, and broadcast final transcripts back to the Accessibility service for focused-field insertion. Successful final dictations can optionally be saved to local-only history for later copying/deletion. The Models screen starts explicit HTTPS model downloads, verifies SHA-256 before storing a downloaded/prepared marker, deletes private model files, and now includes a separate correction-model section where selecting a downloadable Fix model immediately starts its download and offers deletion afterward. The default Danish/multilingual profile uses a locked Parakeet TDT v3 int8 archive with a real GitHub release URL and SHA-256 checksum; alternative compact multilingual and English low-latency profiles can switch to smaller/faster candidate models and auto-start their downloads; Custom now also exposes experimental Whisper tiny/base sherpa-onnx models that can run dictation locally and optionally bias recognition toward Automatic, Danish, English, Spanish, or German. The Whisper language picker is shown only while a Whisper model is selected. The floating overlay now also shows immediate `✨ Fixing` feedback when Fix is pressed. It is still not public-release-ready: Whisper latency/quality and model-based correction both need real device-level quality/performance validation before release. A local-only `ROADMAP.md` file may exist in developer checkouts and is intentionally ignored by git. Tracked planning and release documents live under `docs/`.
